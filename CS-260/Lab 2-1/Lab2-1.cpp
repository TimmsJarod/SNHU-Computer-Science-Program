...
// Added reference to parser header file.
...
vector<Bid> loadBids(string csvPath) {
	//  Defined a vector data structure to hold a collection of bids.
	...
	for (int i = 0; i < file.rowCount(); i++) {
        // Created a data structure to hold data from each row and add to vector
        Bid bid;
        bid.title = file[i][0];
        bid.fund = file[i][8];
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 2:
            ...
            break;
        case 3:
            // Loops and displays the bids read
            for (int i = 0; i < vectorBids.size(); ++i) {
            	displayBid(vectorBids[i]);
            }
            cout << endl;
