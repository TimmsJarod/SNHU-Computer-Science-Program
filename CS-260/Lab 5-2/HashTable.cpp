...
private:
    ...
    struct Node
    {
        ...
        //Initiallize with a bid and a key
...
//Constructor resizes nodes to tableSize
HashTable::HashTable() {
...
unsigned int HashTable::hash(int key) {
    //Implemented logic to calculate a hash value
...
void HashTable::Insert(Bid bid) {
    //Convert bidID to integer to be used as key
    unsigned key = hash(atoi(bid.bidId.c_str()));

    //Try and recieve node using hash key
    Node* oldNode = &(nodes.at(key));

    //If no entry is found, call constructor 
    if (oldNode == nullptr)
    {
...
void HashTable::PrintAll() {
    //Iterates through nodes and finds used nodes to print
    for (unsigned int i = 0; i < nodes.size(); i++) {
        Node* printNode = &(nodes.at(i));
...
Bid HashTable::Search(string bidId) {
    ...
    //If bid not found, return empty
    if (node == nullptr || node->key == UINT_MAX)
    {
        return bid;
    }

    //If node is found, search for key
